...
class BarHorizontalGenerator(BasePlotlyGenerator):
    ...
    def generate(
        ...
        5. Criar trace go.Bar horizontal
        ...
        # Criar trace de barras horizontais
        trace = go.Bar(
            x=values,
            y=categories,
            orientation="h",
        ...
        self.styler.apply_axis_config(
            ...
            y_title=dimension_alias,
            x_type="linear",
            y_type="category",
            y_labels=categories,  # Passar categorias para tratamento de texto longo
        )

        # Ajustar configuracoes especificas de barras horizontais
        fig.update_yaxes(
            showgrid=False,  # Sem grid no eixo Y (categorias)
            autorange="reversed",  # Mantem rankings em ordem decrescente visualmente
        )

        fig.update_xaxes(
            showgrid=True,  # Grid no eixo X (valores)
        )

        # APLICAR CONFIGURAÇÕES ESTÉTICAS CENTRALIZADAS
        aesthetics = BarAesthetics()
        aesthetics.apply_horizontal_bar_style(fig, categories, show_grid=True)
