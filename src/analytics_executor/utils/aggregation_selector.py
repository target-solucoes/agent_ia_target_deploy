...
class AggregationSelector:
    ...
    CATEGORICAL_SQL_TYPES = {
        "VARCHAR", "STRING", "TEXT", "CHAR", "BOOLEAN"
    }
    ...
    def _detect_type_from_schema(self, column_name: str, schema: Dict[str, str]) -> Optional[str]:
        """
    ...
    def get_column_type(self, column_name: str, schema: Optional[Dict[str, str]] = None) -> str:
        """
        Determina o tipo de uma coluna.

        Prioridade:
        1. Configuracao do alias.yaml
        2. Schema SQL (fallback)
        3. Assume categorical como padrao seguro
        ...
        Returns:
            "numeric", "categorical" ou "temporal"
        """
        # Prioridade 1: Configuracao explicita
        type_from_config = self._detect_type_from_config(column_name)
        if type_from_config:
            logger.debug(f"Coluna '{column_name}' identificada como '{type_from_config}' via alias.yaml")
    ...
    def is_numeric_column(self, column_name: str, schema: Optional[Dict[str, str]] = None) -> bool:
        """
    ...
    def is_categorical_column(self, column_name: str, schema: Optional[Dict[str, str]] = None) -> bool:
        """
    ...
    def is_temporal_column(self, column_name: str, schema: Optional[Dict[str, str]] = None) -> bool:
        """
