...
class SemanticKeywords:
    ...
    VARIATION_KEYWORDS = {
        "queda", "aumento", "crescimento", "reducao", "redução", "declinio",
        "declínio", "variacao", "variação", "mudanca", "mudança",
        "evolucao", "evolução", "diferenca", "diferença", "comparacao",
        "comparação", "versus", "vs", "contra"
    }

    # Negative polarity keywords
    NEGATIVE_KEYWORDS = {
        "queda", "reducao", "redução", "declinio", "declínio", "diminuicao",
        "diminuição", "caiu", "caíu", "menor", "pior", "piores",
        "bottom", "ultimo", "últimos", "ultimas", "últimas"
    }

    # Positive polarity keywords
    POSITIVE_KEYWORDS = {
        "aumento", "crescimento", "expansao", "expansão", "cresceu",
        "maior", "melhor", "melhores", "top", "primeiro", "primeiros",
        "primeira", "primeiras", "lider", "líder", "destaque"
    }

    # Temporal comparison keywords
    TEMPORAL_COMPARISON_KEYWORDS = {
        "entre", "de", "para", "ate", "até", "desde", "comparar",
        "variacao", "variação", "vs", "versus", "contra"
    }

    # Temporal keywords (general)
    TEMPORAL_KEYWORDS = {
        "historico", "histórico", "evolucao", "evolução", "tendencia",
        "tendência", "serie", "série", "temporal", "ao longo"
    }

    # Ranking keywords
    RANKING_KEYWORDS = {
        "top", "ranking", "maior", "menor", "melhor", "pior",
        "primeiro", "ultimo", "últimos", "lider", "líder",
        "classificacao", "classificação"
    }

    # Distribution keywords
    DISTRIBUTION_KEYWORDS = {
        "participacao", "participação", "proporcao", "proporção",
        "distribuicao", "distribuição", "percentual", "percent",
        "fatia", "composicao", "composição"
    }
...
class SemanticValidator:
    ...
    def validate(self, anchor: SemanticAnchor, query: str) -> SemanticValidationResult:
        ...
        # ====================================================================
        variation_found = self._find_keywords(query_lower, SemanticKeywords.VARIATION_KEYWORDS)
        if variation_found:
        ...
        # ====================================================================
        negative_found = self._find_keywords(query_lower, SemanticKeywords.NEGATIVE_KEYWORDS)
        if negative_found:
        ...
        # ====================================================================
        positive_found = self._find_keywords(query_lower, SemanticKeywords.POSITIVE_KEYWORDS)
        if positive_found:
        ...
        temporal_pattern_match = any(re.search(pattern, query_lower) for pattern in temporal_patterns)
        ...
        # ====================================================================
        ranking_found = self._find_keywords(query_lower, SemanticKeywords.RANKING_KEYWORDS)
        ...
        ranking_pattern_match = any(re.search(pattern, query_lower) for pattern in ranking_patterns)
