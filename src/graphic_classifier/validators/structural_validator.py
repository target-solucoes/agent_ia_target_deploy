...
class MetricIntentValidator:
    ...
    # Mapeamento de intent -> metricas esperadas
    # FASE 3 - Etapa 3.2: Atualizado para usar novos nomes de intents
    EXPECTED_METRICS = {
        "entity_ranking": [
            "Valor_Vendido",
            "Qtd_Vendida",
        ],  # Anteriormente: product_ranking
        "temporal_comparison_analysis": [
            "Valor_Vendido",
            "Qtd_Vendida",
        ],  # Unificacao de temporal_variation_analysis + month_to_month_comparison
        "temporal_trend": ["Valor_Vendido", "Qtd_Vendida"],
        "proportion_analysis": ["Valor_Vendido", "Qtd_Vendida"],
    }
    ...
    def validate(
        ...
        expected_metrics = MetricIntentValidator.EXPECTED_METRICS.get(intent, [])
